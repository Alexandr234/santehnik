...
"""
Voicer API batch TTS generator (RU / PL).

Что делает:
1. Берёт текстовые сценарии из папки:
   /Users/aleksandrtomilov/Desktop/ПТИЦЫ АВТОМАТИЗАЦИЯ/СЦЕНАРИИ

2. Озвучивает 2 файла строго по очереди:
   RU -> PL

3. Каждый язык озвучивается СВОИМ голосом шаблона (TEMPLATE_UUID) из Voicer API:
     RU -> d7d93654-11ed-45ed-b7c6-21787135ee38
     PL -> c552325d-9b67-43ad-abe6-d82b946c860b
...
4. Сохраняет результат в подпапку:
   /Users/aleksandrtomilov/Desktop/ПТИЦЫ АВТОМАТИЗАЦИЯ/ОЗВУЧКА
   под именами RU и PL (RU.mp3 / PL.mp3 или RU.zip / PL.zip).
...
Запуск:
   ...
   python3 voicer_batch_tts_RU_PL.py
"""
...
SCRIPT_VERSION = "RU_PL_2026_07_02_TEMPLATE_VOICE_PER_LANG"
...
# UUID шаблонов из API — для каждого языка СВОЙ голос.
# Голос / модель / движок / водяной знак / настройки берутся из шаблона.
TEMPLATE_UUID_BY_LANG = {
    "RU": "d7d93654-11ed-45ed-b7c6-21787135ee38",
    "PL": "c552325d-9b67-43ad-abe6-d82b946c860b",
}
...
# Имена выходных файлов (без расширения) для каждого языка.
OUTPUT_NAMES = {
    "RU": "RU",
    "PL": "PL",
}
...
def template_uuid_for_language(lang: str) -> str:
    uuid = TEMPLATE_UUID_BY_LANG.get(lang)
    if not uuid:
        raise RuntimeError(f"Не задан TEMPLATE_UUID для языка {lang}.")
    return uuid


def output_base_for_language(lang: str) -> Path:
    """Имя выходного файла без расширения: RU / PL."""
    name = OUTPUT_NAMES.get(lang, lang)
...
def build_task_payload(text: str, template_uuid: str) -> dict:
    """
...
def create_tts_task(base_url: str, text: str, template_uuid: str) -> int:
    payload = build_task_payload(text, template_uuid)
...
def process_one_language(base_url: str, lang: str) -> None:
    ...
    task_id = create_tts_task(base_url, text, template_uuid)
...
def main() -> None:
    ...
    for lang in LANG_ORDER:
        log(f"[TEMPLATE:{lang}] {TEMPLATE_UUID_BY_LANG.get(lang)}")
