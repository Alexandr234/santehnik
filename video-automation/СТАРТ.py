...
Делает всё сам и в правильном порядке:
 ...
 10. готовит тексты описаний под каждый ролик;
 11. показывает, что получилось и где лежит.
...
def main() -> None:
    ...
    # --- Шаг 2: фото ---
    title("ШАГ 3 из 6. ГЕНЕРАЦИЯ ФОТО")
    ...
    # --- Шаг 3: оживление ---
    title("ШАГ 4 из 6. ОЖИВЛЕНИЕ ФОТО")
    if not run_step("step3_animate.py", ["--limit", str(count)]):
        sys.exit(1)

    # --- Шаг 4: монтаж ---
    title("ШАГ 5 из 6. МОНТАЖ")
    if not run_step("step4_montage.py", ["--limit", str(count)]):
        sys.exit(1)

    # --- Шаг 5: описания под ролики ---
    title("ШАГ 6 из 6. ОПИСАНИЯ ПОД РОЛИКИ")
    run_step("step5_description.py", [])
    ...
    print(f"\n  Тексты описаний под ролики лежат в: {config.DESCRIPTIONS_DIR}")
    print(
