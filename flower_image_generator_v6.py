...
def build_payload(prompt: str, operation: str) -> Dict[str, Any]:
    ...
    if operation == "grok_image_generate":
        ...
        if q in {"speed", "quality"}:
            payload["quality"] = q
    # grok игнорирует seed. Для остальных: если seed не задан жёстко (--seed / FAST_GEN_SEED),
    # даём КАЖДОЙ картинке свой случайный seed — иначе кадры сходятся к одному и тому же «персонажу»/лицу.
    if operation != "grok_image_generate":
        seed_val = IMAGE_SEED if IMAGE_SEED is not None else random.randint(0, SEED_MAX)
        payload["seed"] = min(max(0, seed_val), SEED_MAX)
