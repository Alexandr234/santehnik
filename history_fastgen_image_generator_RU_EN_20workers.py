...
STYLE_GUARD_RU = (
    "СТРОГО ЕДИНЫЙ СТИЛЬ: flat 2D hand-drawn educational explainer cartoon, "
    "простая рисованная образовательная иллюстрация, clean composition, flat pastel colors, "
    "тонкие тёмные контуры, минималистичный фон. "
    "Если в кадре есть люди, они должны быть именно тонкими stickman-человечками: "
    "очень тонкие руки и ноги, маленькое тонкое тело, крупная круглая голова, "
    "очень простой doodle-вид. "
    "НЕ фотореализм, НЕ фотография, НЕ 3D, НЕ CGI, НЕ live-action, НЕ реалистичная анатомия. "
)

STYLE_GUARD_EN = (
    "STRICT UNIFIED STYLE: flat 2D hand-drawn educational explainer cartoon, "
    "clean composition, flat pastel colors, thin dark outlines, minimal background. "
    "If people appear, they must be thin stickman figures: very thin arms and legs, "
    "a small thin body, a large round head, and a simple doodle look. "
    "NOT photorealistic, NOT photography, NOT 3D, NOT CGI, NOT live-action, NOT realistic anatomy. "
)
...
def build_fallback_prompt(voice_text: str, labels: str, block_type: str, lang: str) -> str:
    if lang == "EN":
        base = (
            "Simple hand-drawn 2D educational cartoon illustration, thin dark outlines, flat pastel colors, "
            "clean composition, minimal background. If people appear, they must be thin stickman figures: "
            "large round head, very thin arms and legs, small thin body. "
            "Not photorealistic, not 3D, not CGI. "
        )
        if block_type.lower() in {"comparison", "infographic", "scheme", "map", "diagram"}:
            base += "The scene may use a simple explanatory layout with arrows or visual comparison only if truly needed. "
        scene = f"Scene: clearly visualize the narrator's phrase: {voice_text}. "
        scene += f"Allowed labels inside the image: {labels}. " if labels else "No text inside the image. "
        return base + scene

    base = (
        "простая рисованная 2D иллюстрация в стиле образовательного мультфильма, тонкие тёмные контуры, "
        "плоские пастельные цвета, чистая композиция, минималистичный фон. "
        "Если в кадре есть люди, они должны быть очень тонкими stickman-человечками: большая круглая голова, "
        "тонкие палочные руки и ноги, маленькое узкое тело. "
        "Без фотореализма, без 3D, без CGI. "
    )
    if block_type.lower() in {"comparison", "infographic", "scheme", "map", "diagram"}:
        base += "Сцена может быть простой объясняющей схемой или сравнением только если это реально нужно по смыслу. "
    scene = f"Сцена: ясно визуализировать фразу диктора: {voice_text}. "
    scene += f"Разрешённые подписи внутри изображения: {labels}. " if labels else "Без текста внутри изображения. "
    return base + scene
...
def strengthen_prompt(
    ...
    if lang == "EN":
        ...
        final_prompt = (
            ...
            + "MAIN PROMPT: "
            + prompt
            + " Final reminder: preserve a unified flat hand-drawn 2D educational cartoon style; "
            + "if people appear, draw them only as thin stickman figures with large round heads and thin limbs."
        )
    ...
    final_prompt = (
        ...
        + "ОСНОВНОЙ ПРОМПТ: "
        + prompt
        + " Финальное напоминание: сохранить единый плоский рисованный 2D educational cartoon style; "
        + "если есть люди — рисовать их только как тонких stickman-человечков с большой круглой головой и тонкими конечностями."
    )
    return clean_text(final_prompt)
