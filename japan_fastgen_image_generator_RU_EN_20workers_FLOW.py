...
# -*- coding: utf-8 -*-
"""
media_gen_api V6 image generator for ЯПОНИЯ АВТОМАТИЗАЦИЯ.
MULTI-LANG version: RU + EN.

FLOW-версия (аналог flower-скрипта, но генерация идёт через провайдера flow).

Что делает:
1) Читает промпты по блокам из:
   - /Users/aleksandrtomilov/Desktop/ЯПОНИЯ АВТОМАТИЗАЦИЯ/ПРОМПТЫ/ru_RU_prompts.txt
   - /Users/aleksandrtomilov/Desktop/ЯПОНИЯ АВТОМАТИЗАЦИЯ/ПРОМПТЫ/en_EN_prompts.txt
...
8) Отличие FLOW-версии от FLOWER-версии:
   - протокол V6 и endpoints не меняются:
       * генерация -> POST /api/v6/generations
       * статус     -> GET  /api/v6/generations/{generation_id}
       * статусы    -> queued / running / succeeded / failed
       * результат  -> results[] как download_url (файл) либо inline data (data URI);
   - меняется только canonical operation id: вместо провайдера flower
     (operation flower_image_generate) используется провайдер flow.
     У flow image-генерация идёт через nano-banana модели:
       * nano_banana_pro_image_generate      (flow/nano-banana-pro)   <- по умолчанию
       * nano_banana_2_image_generate        (flow/nano-banana-2)
       * nano_banana_2_lite_image_generate   (flow/nano-banana-2-lite)
     Operation можно переопределить через env FAST_GEN_IMAGE_OPERATION.

Запуск:
   python3 japan_fastgen_image_generator_RU_EN_20workers_FLOW.py
   python3 japan_fastgen_image_generator_RU_EN_20workers_FLOW.py --lang RU
   python3 japan_fastgen_image_generator_RU_EN_20workers_FLOW.py --lang EN
   python3 japan_fastgen_image_generator_RU_EN_20workers_FLOW.py --workers 20 --no-skip-existing
   FAST_GEN_IMAGE_OPERATION=nano_banana_2_image_generate python3 japan_fastgen_image_generator_RU_EN_20workers_FLOW.py
"""
...
# Canonical V6 operation id для провайдера FLOW (см. GET /api/v6/capabilities).
# У flow image-генерация идёт через nano-banana модели. По умолчанию nano-banana-pro.
# Альтернативы: nano_banana_2_image_generate, nano_banana_2_lite_image_generate.
# Переопределяется через env FAST_GEN_IMAGE_OPERATION.
...
STYLE_GUARD_RU = (
    "СТРОГО ЕДИНЫЙ СПОКОЙНЫЙ СТИЛЬ: тёплая уютная 2D редакторская иллюстрация в духе премиального ролика "
    "об осознанной жизни и японском минимализме (спокойный lo-fi cozy explainer про порядок, дом и привычки). "
    "Мягкие плавные формы; тонкий мягкий тёмно-коричневый контур ровной толщины (или вовсе без жёсткого контура). "
    "Аккуратная плоская заливка с мягкими нежными тенями и деликатным тёплым свето-градиентом — "
    "не жёсткая плоская заливка, но и не фотореализм. "
    "Тёплая землистая приглушённая палитра: кремово-бежевые и молочные стены, тёплая охра и горчичный, "
    "терракота и мягкий ржаво-оранжевый, шалфейный и оливково-зелёный, тёплое дерево, "
    "пыльно-бирюзовый и приглушённо-синий акцент. "
    "Человек — реалистичный, но упрощённый и обаятельный персонаж: нормальные человеческие пропорции, "
    "спокойное выразительное лицо, мягкие волнистые волосы, простая уютная одежда; НИКОГДА не стикмен и не мультяшный шарж. "
    "Предметы и растения — узнаваемые, с правильными пропорциями мягкие формы с такой же деликатной заливкой и тенью. "
    "Уютный минималистичный интерьер дома в японском духе: чистое спокойное пространство, деревянная мебель, "
    "немного предметов, комнатные растения, мягкий естественный свет, много воздуха и свободного места. "
    "Мягкое рассеянное освещение, деликатные тени, спокойная дышащая композиция. "
    "НЕ неон, НЕ кислотные цвета, НЕ жёсткие чёрные контуры, НЕ мультяшный стикмен-стиль, НЕ каракуль, "
    "НЕ фотореализм, НЕ фотография, НЕ 3D, НЕ CGI, НЕ аниме, НЕ манга, НЕ live-action. "
)

STYLE_GUARD_EN = (
    "STRICT UNIFIED CALM STYLE: warm cozy 2D editorial illustration in the spirit of a premium mindful-living "
    "and Japanese-minimalism explainer (calm lo-fi cozy explainer about order, the home and habits). "
    "Soft rounded shapes; a thin soft dark-brown outline of even weight (or no hard outline at all). "
    "Clean flat fills with soft delicate shadows and a subtle warm light gradient — "
    "not harsh flat fill, but not photoreal either. "
    "Warm earthy muted palette: creamy beige and milky walls, warm ochre and mustard, "
    "terracotta and soft rust-orange, sage and olive green, warm wood, dusty teal and muted blue accents. "
    "The human is a realistic but simplified, likeable character: natural human proportions, "
    "a calm expressive face, soft wavy hair, simple cozy clothing; NEVER a stick figure and never a cartoon caricature. "
    "Objects and plants are recognizable, correctly proportioned soft shapes with the same delicate fill and shadow. "
    "A cozy minimalist Japanese-inspired home interior: clean calm space, wooden furniture, few objects, "
    "houseplants, soft natural light, lots of breathing room and negative space. "
    "Soft diffuse lighting, delicate shadows, calm breathable composition. "
    "NOT neon, NOT acid colors, NOT hard black outlines, NOT cartoon stick-figure style, NOT scribble, "
    "NOT photorealistic, NOT photography, NOT 3D, NOT CGI, NOT anime, NOT manga, NOT live-action. "
)
...
def looks_like_real_prompt(prompt: str) -> bool:
    ...
    if len(prompt) < 100:
        return False
    prompt_markers = [
        "сцена:", "2d", "рисован", "иллюстрац", "уютн", "минимал", "интерьер", "спокой",
        "editorial", "cozy", "illustration", "scene:", "japanese", "minimalist", "interior", "calm"
    ]
...
def build_fallback_prompt(voice_text: str, labels: str, block_type: str, lang: str) -> str:
    if lang == "EN":
        base = (
            "Warm cozy 2D editorial illustration in the spirit of a mindful-living / Japanese-minimalism explainer, "
            "soft rounded shapes, thin soft dark-brown outline (or no hard outline), clean flat fills with soft "
            "delicate shadows and a subtle warm light gradient, "
            "warm earthy muted palette (creamy beige, warm ochre, terracotta, sage green, warm wood, dusty teal), "
            "a cozy minimalist Japanese-inspired home interior with lots of breathing room. "
            "The human is a realistic but simplified, likeable character: natural proportions, calm expressive face, "
            "soft wavy hair, simple cozy clothing, never a stick figure. "
            "Objects and plants are recognizable soft shapes with the same delicate fill and shadow. "
            "Not neon, not hard black outlines, not photorealistic, not 3D, not CGI, not anime. "
        )
        if block_type.lower() in {"contrast", "before_after", "concept", "object", "comparison", "infographic", "scheme", "diagram"}:
            base += "Use a calm, uncluttered layout: one clear idea, gentle composition, minimal or no text. "
        scene = f"Scene: calmly visualize the narrator's phrase: {voice_text}. "
        scene += f"Allowed subtle labels inside the image: {labels}. " if labels else "No text inside the image. "
        return base + scene

    base = (
        "тёплая уютная 2D редакторская иллюстрация в духе ролика об осознанной жизни и японском минимализме, "
        ...
        "уютный минималистичный интерьер дома в японском духе с большим количеством воздуха. "
        "Человек — реалистичный, но упрощённый обаятельный персонаж: нормальные пропорции, спокойное выразительное лицо, "
        "мягкие волнистые волосы, простая уютная одежда, никогда не стикмен. "
        "Предметы и растения — узнаваемые мягкие формы с такой же деликатной заливкой и тенью. "
        "Не неон, без жёстких чёрных контуров, без фотореализма, без 3D, без CGI, не аниме. "
    )
    if block_type.lower() in {"contrast", "before_after", "concept", "object", "comparison", "infographic", "scheme", "diagram"}:
        base += "Спокойная, незагромождённая раскладка: одна ясная мысль, мягкая композиция, минимум текста или без текста. "
    scene = f"Сцена: спокойно визуализировать фразу диктора: {voice_text}. "
    scene += f"Разрешённые деликатные подписи внутри изображения: {labels}. " if labels else "Без текста внутри изображения. "
    return base + scene
...
def strengthen_prompt(
    ...
    if lang == "EN":
        ...
        final_prompt = (
            ...
            + "MAIN PROMPT: "
            + prompt
            + " Final reminder: keep the unified warm cozy calm editorial illustration style — soft rounded shapes, "
            + "thin soft brown outline or none, soft delicate shadows and warm light, warm earthy muted palette; "
            + "humans as realistic but simplified likeable characters (never stick figures), a cozy minimalist Japanese-inspired home."
        )
    ...
    final_prompt = (
        ...
        + "ОСНОВНОЙ ПРОМПТ: "
        + prompt
        + " Финальное напоминание: сохранить единый тёплый уютный спокойный редакторский стиль иллюстрации — мягкие плавные формы, "
        + "тонкий мягкий коричневый контур или без контура, мягкие деликатные тени и тёплый свет, тёплая землистая приглушённая палитра; "
        + "люди — реалистичные, но упрощённые обаятельные персонажи (никогда не стикмены), уютный минималистичный интерьер в японском духе."
    )
    return clean_text(final_prompt)
...
def start_flow_image(prompt: str) -> str:
    ...
    data = request_with_retries_post(url, payload, label="FLOW IMAGE GENERATE")
...
def generate_image_flow(prompt: str) -> Tuple[Dict[str, Any], str]:
...
def generate_image_until_done(item: PromptItem, out_path: Path, lang: str) -> dict:
    ...
    while True:
        ...
        try:
            ...
            result_item, generation_id = generate_image_flow(item.final_prompt)
...
def main() -> None:
    global BASE_URL, SKIP_EXISTING, OP_IMAGE_GENERATE

    parser = argparse.ArgumentParser(description="Generate RU/EN images from prompt files with media_gen_api V6 (FLOW provider).")
    ...
    parser.add_argument("--operation", default=OP_IMAGE_GENERATE,
                        help="Canonical V6 operation id провайдера flow. По умолчанию nano_banana_pro_image_generate")
    ...
    OP_IMAGE_GENERATE = args.operation
    ...
    log("media_gen_api V6 image generator — RU+EN (FLOW provider)")
