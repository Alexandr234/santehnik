...
Модель ЖЁСТКО зафиксирована: всегда используется flow (nano_banana_pro_image_generate).
Скрипт НЕ пытается подбирать/определять «рабочую» модель — просто берёт flow-модель и гонит на ней.
Переопределить можно через env FAST_GEN_IMAGE_OPERATION
...
Быстрый запуск:
  python3 flow_image_generator_v6_3.py

Флаги:
  --workers auto|max|N     потоки. auto = лимит аккаунта − запас (по умолчанию)
  --lang DE PL RU          только нужные языки
  --seed N                 фикс. seed, -1 = случайный
  --upscale                2x-апскейл (удваивает кредиты)
...
# API-ключ больше НЕ хранится в коде. Задай через переменную окружения:
...
# Модель зафиксирована — всегда flow (nano-banana). Никакого автоподбора.
FLOW_OPERATION = "nano_banana_pro_image_generate"

# Текущая операция: всегда flow (можно переопределить через env, но по умолчанию nano-banana-pro).
V6_OPERATION = os.getenv("FAST_GEN_IMAGE_OPERATION", "").strip() or FLOW_OPERATION
...
def build_payload(prompt: str, operation: str) -> Dict[str, Any]:
    ...
    # seed на flow/nano-banana поддерживается «когда модель это умеет» и может привести к HTTP 400,
    # поэтому случайный seed КАЖДОЙ картинке НЕ навязываем. Шлём seed только если он задан ЯВНО
    # (--seed / FAST_GEN_SEED) — тогда это осознанный выбор пользователя.
    if IMAGE_SEED is not None:
        payload["seed"] = min(max(0, IMAGE_SEED), SEED_MAX)
    # flow-картинки поддерживают 2x-апскейл через generation_config.upscale.type='2x'.
    if FLOW_UPSCALE_2X:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Генерирует DE/PL/RU картинки ИЗ ТЕКСТА через Fast-Gen V6 (модель flow / nano-banana), "
                    "без персонажа, один общий пул потоков."
    )
    ...
    log(f"[МОДЕЛЬ] {V6_OPERATION} (зафиксировано)")
    log(f"Fast-Gen V6 | operation={V6_OPERATION} | text-to-image, без персонажа | aspect_ratio={_resolve_aspect_ratio()}")
