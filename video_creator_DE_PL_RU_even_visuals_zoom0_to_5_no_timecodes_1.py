# video_creator_DE_PL_RU_even_visuals_zoom0_to_5_no_timecodes.py
# -*- coding: utf-8 -*-
...
# Резервные (фиксированные) имена озвучек.
# Используются ТОЛЬКО если в папке не удалось найти файл по маркеру RU / DE / PL в имени.
AUDIO_FILE_BY_LANG = {
    "DE": AUDIO_DIR / "scenario_de.mp3",
    "PL": AUDIO_DIR / "scenario_pl.mp3",
    "RU": AUDIO_DIR / "scenario_ru.mp3",
}

# Маркеры языка в имени файла озвучки.
# Скрипт сам найдёт файлы вида "RU.mp3", "scenario_ru.mp3", "озвучка-RU.wav",
# "german_voice.m4a", "PL_final.mp3" и т.п.
LANG_FILENAME_CUES = {
    "RU": ["ru", "rus", "russian", "рус", "русск", "русский"],
    "PL": ["pl", "pol", "polski", "polish", "польск", "польский"],
    "DE": ["de", "deu", "ger", "german", "deutsch", "немец", "немецкий"],
}
...
# Плавный зум на каждый визуал:
# начало сегмента = 0% дополнительного приближения;
# конец сегмента = +5% приближения.
# То есть картинка/визуал плавно приближается от 1.00 до 1.05 в течение своей длительности.
ZOOM_START = 1.00
ZOOM_END = 1.05
...
def _token_matches_cue(token: str, cue: str) -> bool:
    """Короткие коды (ru/pl/de) — только точное совпадение токена.
    Более длинные слова (rus, german, deutsch, русск...) — совпадение по началу токена."""
    if len(cue) <= 2:
        return token == cue
    return token == cue or token.startswith(cue)


def detect_audio_language(path: Path) -> Optional[str]:
    """Определяет язык озвучки по имени файла (маркеры RU / DE / PL и их варианты).

    Имя разбивается на токены по любым не-буквенно-цифровым символам, поэтому
    'RU.mp3', 'scenario_ru.mp3', 'озвучка-RU.wav', 'PL_final.mp3', 'german_voice.m4a'
    все определяются корректно."""
    stem = path.stem.lower()
    tokens = [t for t in re.split(r"[^0-9a-zа-яё]+", stem) if t]
    if not tokens:
        return None
    for lang in LANGUAGES:
        for cue in LANG_FILENAME_CUES.get(lang, []):
            for token in tokens:
                if _token_matches_cue(token, cue):
                    return lang
    return None
...
def resolve_audio_by_language(audio_dir: Path) -> dict[str, Path]:
    """Сканирует папку озвучек и сопоставляет каждому языку файл по маркеру в имени.

    Если для языка нашлось несколько подходящих файлов, берётся первый по
    естественной сортировке. Если по маркеру ничего не найдено, язык
    останется без записи (позже сработает резервное фиксированное имя)."""
    mapping: dict[str, Path] = {}
    for path in list_audio_files(audio_dir):
        lang = detect_audio_language(path)
        if lang and lang not in mapping:
            mapping[lang] = path
            log(f"Озвучка {lang}: найден файл по имени -> {path.name}")
    return mapping
...
################################################
# 3. РЕНДЕР КАДРОВ: ВИДЕО/КАРТИНКИ + ПЛАВНЫЙ ЗУМ 0→5% + БЕЗ ПЕРЕХОДОВ
################################################
...
def zoom_for_frame(frame_index: int, total_frames: int) -> float:
    """
    Плавное приближение внутри каждого визуального сегмента.

    frame_index = 0             -> ZOOM_START, то есть 0% зума.
    последний кадр сегмента     -> ZOOM_END, то есть +5% зума.
    """
    if total_frames <= 1:
        return ZOOM_START
...
def iter_image_frames(segment: Segment, total_frames: int) -> Iterator[np.ndarray]:
    ...
    # Сначала подгоняем картинку под 1920x1080, затем на каждом кадре
    # добавляем плавный зум от 0% до 5% в рамках длительности этого сегмента.
...
def build_video_for_language(lang: str, audio_path: Path, visual_dir: Path) -> bool:
    ...
    try:
        ...
        log("Переходы: отключены. Зум: плавное приближение 0→5% на каждом визуале. Субтитры: отключены.")
...
def resolve_audio_path(lang: str, detected: dict[str, Path]) -> Optional[Path]:
    """Сначала берём файл, найденный по маркеру RU / DE / PL в имени.
    Если не нашли — пробуем резервное фиксированное имя из AUDIO_FILE_BY_LANG."""
    path = detected.get(lang)
    if path and path.exists():
        return path

    fallback = AUDIO_FILE_BY_LANG.get(lang)
    if fallback and fallback.exists():
        log(f"Озвучка {lang}: маркер в имени не найден, использую резервное имя {fallback.name}")
        return fallback
...
def main() -> None:
    ...
    # Ищем озвучки по именам с маркерами RU / DE / PL.
    detected_audio = resolve_audio_by_language(AUDIO_DIR)
    if not detected_audio:
        log("⚠️ В папке озвучек не найдено файлов с маркерами RU / DE / PL в имени. "
            "Попробую резервные фиксированные имена.")

    jobs = []
    for lang in LANGUAGES:
        audio_path = resolve_audio_path(lang, detected_audio)
        visual_dir = VISUAL_DIR_BY_LANG[lang]

        if audio_path is None:
            log(f"⚠️ Пропускаю {lang}: не найдена озвучка (нет файла с маркером {lang} в имени "
                f"и нет резервного {AUDIO_FILE_BY_LANG.get(lang)})")
            continue
