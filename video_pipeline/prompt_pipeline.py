...
STYLE_PREFIX_SHORT = {
    "ru": (
        "Плоский 2D векторный мультфильм в стиле вирусного образовательного YouTube-ролика: "
        ...
        "Люди — простые стикмены (белая круглая голова, чёрные точки-глаза, тонкие руки-ноги одной линией), "
        "животные и предметы — узнаваемые плоские мультяшные формы с таким же жирным чёрным контуром. "
        "Уверенные чёткие линии, не карандаш и не каракуль. Не фотореализм, не 3D, не пастель"
    ),
    "en": (
        "Flat 2D vector cartoon in the style of a viral educational YouTube explainer: "
        "bold even black outlines, fully flat solid color fills with no gradients and no soft shading, "
        "warm earthy muted palette (sandy beige, orange sky, olive green, warm browns, ochre). "
        "Humans are simple stick figures (white round head, black dot eyes, thin single-line limbs); "
        "animals and objects are recognizable flat cartoon shapes with the same bold black outline. "
        "Confident clean linework, not pencil, not scribble. Not photorealistic, not 3D, not pastel"
    ),
}
...
- Плоский 2D векторный мультфильм в стиле вирусного образовательного YouTube-ролика (объясняющая анимация про историю/эволюцию/науку)
- Жирные ровные ЧЁРНЫЕ контуры одинаковой толщины у каждого объекта; уверенные чёткие линии (НЕ карандаш, НЕ эскиз, НЕ кривой каракуль)
- Полностью плоская заливка ровными цветами: без градиентов, без мягких теней, без текстуры; максимум одна плоская тёмная тень-пятно
- Тёплая землистая приглушённая палитра: песочно-бежевая земля, оранжевое небо, оливковый и травяной зелёный, тёплые коричневые, охра (НЕ пастель, НЕ кислотные цвета)
- Человек — ВСЕГДА один и тот же стикмен: белая круглая голова с чёрным контуром, круглые белые глаза с чёрными точками-зрачками, простые выразительные брови и рот (можно эмоции, открытый рот, зубы), тонкие руки и ноги одной чёрной линией, при необходимости лохматые волосы сплошным коричневым пятном. Без проработанного тела, без мышц, без одежды
- Животные и предметы — узнаваемые, с правильными пропорциями плоские мультяшные формы с таким же жирным чёрным контуром и плоской заливкой (лев выглядит как лев), НИКОГДА не стикмены
- Полноценный, но простой плоский ФОН по смыслу сцены (саванна: оранжевое небо + песочная земля + кустики травы + акации; пещера: серый камень; и т.п.), чёткая линия горизонта
- Не фотореализм, не 3D, не CGI, не живопись, не аниме
...
ЧЕТЫРЕ ТИПА КАДРОВ:
1. scene — обычная сюжетная сцена с полным плоским фоном (по умолчанию, ~80% кадров)
2. big_text — только крупное слово или цифра ЖИРНЫМИ ЗАГЛАВНЫМИ буквами на простом фоне (для статистики, ключевых слов)
3. cross_out — нарисовать объект и перечеркнуть большим жирным чёрным крестом X (для "нет X", "без X")
4. simple_chart — простое сравнение или таймлайн: 1-2 плоские фигуры + чёрная стрелка + жирные заглавные подписи (для причинно-следственных связей и хронологии)

ТЕКСТ ВНУТРИ ИЗОБРАЖЕНИЯ:
- Только русский язык, только кириллица, ЖИРНЫМИ ЗАГЛАВНЫМИ буквами
...
- scene_prompt: 5-15 слов, конкретно ЧТО нарисовать и в каком окружении, без инструкций по стилю
...
- Flat 2D vector cartoon in the style of a viral educational YouTube explainer (history / evolution / science explainer animation)
- Bold, even-weight BLACK outlines of uniform thickness on every shape; confident clean linework (NOT pencil, NOT sketchy, NOT crooked scribble)
- Fully flat solid color fills: no gradients, no soft shading, no texture; at most one flat darker shadow shape
- Warm earthy muted palette: sandy beige ground, orange sky, olive and grass green, warm browns, ochre (NOT pastel, NOT neon)
- The human is ALWAYS the same stick figure: a plain white round head with a black outline, round white eyes with black dot pupils, simple expressive eyebrows and mouth (emotions, open mouth, teeth allowed), thin single-line black stick arms and legs, optional shaggy solid-brown hair. No detailed body, no muscles, no clothes
- Animals and objects are recognizable, correctly proportioned flat cartoon shapes with the same bold black outline and flat fill (a lion looks like a lion), NEVER stick figures
- A complete but simple flat-color BACKGROUND that fits the scene (savanna: orange sky + tan ground + grass tufts + acacia trees; cave: grey rock; etc.), with a clear horizon line
- Not photorealistic, not 3D, not CGI, not painterly, not anime
...
FOUR FRAME TYPES:
1. scene — regular narrative scene with a full flat background (default, ~80% of frames)
2. big_text — only a large word or number in BOLD UPPERCASE on a plain background (for stats, key words)
3. cross_out — draw the object then cross it out with a large bold black X (for "no X", "without X")
4. simple_chart — simple comparison or timeline: 1-2 flat figures + a black arrow + bold uppercase labels (for cause/effect and chronology)

TEXT INSIDE IMAGE:
- English only, in BOLD UPPERCASE
...
- scene_prompt: 5-15 words, concretely WHAT to draw and in what setting, no style instructions
...
def compose_final_prompt(
    ...
    if frame_type == "big_text":
        if language_code == "ru":
            return (
                f"{style}. "
                f"Только одно ключевое слово или цифра, ОЧЕНЬ КРУПНО, ЖИРНЫМИ ЗАГЛАВНЫМИ чёрными буквами по центру, "
                f"на простом плоском фоне из землистой палитры, без персонажей и лишних деталей. "
                f"{scene_prompt}."
            )
        return (
            f"{style}. "
            f"Only one key word or number, VERY LARGE, in BOLD BLACK UPPERCASE letters centered, "
            f"on a plain flat earthy-colored background, no characters, no extra detail. "
            f"{scene_prompt}."
        )
    ...
    if frame_type == "cross_out":
        if language_code == "ru":
            return (
                f"{style}. "
                f"Нарисовать объект чётко в плоском мультяшном стиле, затем перечеркнуть его большим жирным чёрным крестом X. "
                f"Без текста внутри изображения. "
                f"{scene_prompt}."
            )
        return (
            f"{style}. "
            f"Draw the object clearly in flat cartoon style, then cross it out with a large bold black X. "
            f"No text inside the image. "
            f"{scene_prompt}."
        )

    if frame_type == "simple_chart":
        label_part = (
            (f" Короткие подписи ЖИРНЫМИ ЗАГЛАВНЫМИ кириллицей: {labels}." if labels else " Без текста.")
            if language_code == "ru"
            else (f" Short labels in BOLD UPPERCASE: {labels}." if labels else " No text.")
        )
        if language_code == "ru":
            return (
                f"{style}. Простое сравнение или таймлайн: 1-2 плоские фигуры и чёрная стрелка, одна идея, максимум 2 стрелки. "
                f"{scene_prompt}.{label_part}"
            )
        return (
            f"{style}. Simple comparison or timeline: 1-2 flat figures and a black arrow, one idea, at most 2 arrows. "
            f"{scene_prompt}.{label_part}"
        )

    # Default: scene
    scene_bg = (
        " Полноценный, но простой плоский фон/окружение по смыслу сцены с чёткой линией горизонта."
        if language_code == "ru"
        else " A complete but simple flat-color background/setting that fits the scene, with a clear horizon line."
    )
    no_text = (
        (f" Подписи ЖИРНЫМИ ЗАГЛАВНЫМИ кириллицей: {labels}." if labels else " Без текста внутри изображения.")
        if language_code == "ru"
        else (f" Labels in BOLD UPPERCASE: {labels}." if labels else " No text inside the image.")
    )
