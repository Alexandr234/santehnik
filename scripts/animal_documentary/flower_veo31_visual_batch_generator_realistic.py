...
Пайплайн для каждого промпта остался прежним по смыслу:
   текст промпта -> flower_image_generate (стартовый кадр)
...
#   image из текста — flower_image_generate (стартовый кадр);
...
def generate_image_until_done(item: PromptItem, out_path: Path) -> dict:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            log(f"[IMAGE/FLOWER] {out_path.name} | attempt {attempt}/{MAX_ATTEMPTS}")
...
def generate_video_until_done(item: PromptItem, out_path: Path, locale_dir: Path) -> dict:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            log(f"[VIDEO/FLOWER] {out_path.name} | attempt {attempt}/{MAX_ATTEMPTS}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate RU/GE/PL/ES visuals from exact prompt files via media_gen_api V6 (Flower image + Flow video)."
    )
    parser.add_argument(
        "--prompts-root",
