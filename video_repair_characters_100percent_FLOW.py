...
Провайдер: flow (картинки — nano-banana, видео — flow-video).
  • Картинки: operation nano_banana_pro_image_generate (flow/nano-banana-pro).
  • Видео:    operation flow_video_from_keyframes (flow/flow-video-fast), image->video
              через keyframes (inputs[0] = стартовый кадр, флаг keyframes=true).

  Всё переопределяется через окружение:
    FAST_GEN_IMAGE_OPERATION  (по умолч. nano_banana_pro_image_generate;
                               альт.: nano_banana_2_image_generate, nano_banana_2_lite_image_generate)
    FAST_GEN_VIDEO_OPERATION  (по умолч. flow_video_from_keyframes;
                               альт.: flow_video_lite_from_keyframes, flow_ultra_video_from_keyframes,
                               flow_video_quality_from_keyframes, flow_video_ultra_lite_from_keyframes)
    FAST_GEN_VIDEO_KEYFRAMES=1  (flow: inputs[0] — стартовый кадр; по умолч. включено)
    FAST_GEN_VIDEO_ULTRA=0      (flow ultra-video: ultra-лимиты/1080p; по умолч. выключено)

Что делает (две фазы):

  ФАЗА 1 — ДОБИВКА КАРТИНОК.
    Для каждой сцены, у которой есть промпт, но НЕТ картинки в КАРТИНКИ/ (частая причина —
    картинку срезал контент-фильтр), генерирует картинку через IMAGE_OPERATION (flow / nano-banana)
    с той же логикой эскалации, что и видео:
  ...
  ФАЗА 2 — ДОБИВКА ВИДЕО.
    ...
    КАРТИНКУ новым безопасным/анонимизированным промптом (IMAGE_OPERATION, flow / nano-banana), а затем
    оживляет уже новую картинку. Старая картинка бэкапится в КАРТИНКИ/_regen_backup/.

  ВАЖНО ПРО СТИЛЬ (исправление):
    Раньше при срабатывании контент-фильтра скрипт уходил в санитайз/фолбэк-промпты, в которых
    буквально стояло слово "illustration" ("documentary illustration") и терялись исходные
    указания фотореализма — поэтому часть кадров выходила РИСОВАННОЙ (акварель/скетч), а не
    фотореалистичной. Теперь КАЖДЫЙ промпт картинки перед отправкой проходит через
    `enforce_photoreal()`: он вырезает слова-рисунки (illustration/drawing/painting/watercolor/…)
    и ВСЕГДА добавляет фотореалистичный анкер (STYLE_ANCHOR). LLM-переписывание тоже теперь
    обязано сохранять фотореалистичный медиум. Итог: стиль остаётся фотореалистичным на ЛЮБОЙ
    ступени лестницы — включая безопасный fallback.
...
Запуск:
   ...
   python video_repair_characters_100percent_FLOW.py                 # картинки + видео
   python video_repair_characters_100percent_FLOW.py --dry-run
   python video_repair_characters_100percent_FLOW.py --images-only    # только добивка картинок
   python video_repair_characters_100percent_FLOW.py --videos-only    # только добивка видео
   python video_repair_characters_100percent_FLOW.py --no-image-regen # без переделки картинок в fallback
   python video_repair_characters_100percent_FLOW.py --limit 20

   # Свой стиль (по умолчанию фотореализм). Например, если хочешь наоборот РИСОВАННЫЙ стиль:
   export FAST_GEN_STYLE_ANCHOR="hand-painted watercolor illustration, storybook style"
   export FAST_GEN_ENFORCE_PHOTOREAL=0     # выключить принудительный фотореализм
"""
...
# Canonical V6 operation id для image->video на flow (flow-video).
# На flow "оживление картинки" = keyframes: inputs[0] — стартовый кадр.
...
# Canonical V6 operation id для генерации картинок на flow (nano-banana).
IMAGE_ASPECT_RATIO = os.getenv("FAST_GEN_IMAGE_ASPECT_RATIO", ASPECT_RATIO)
IMAGE_OPERATION = os.getenv("FAST_GEN_IMAGE_OPERATION", "nano_banana_pro_image_generate")
...
# flow-video: keyframes (inputs[0] — стартовый кадр) и ultra (ultra-лимиты/1080p).
# Для image->video keyframes по умолчанию ВКЛючены — это и есть "оживление картинки".
# (_env_bool определён ниже, поэтому здесь парсим окружение инлайном.)
...
# =========================================================
# СТИЛЬ КАРТИНКИ (ФОТОРЕАЛИЗМ) — КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ
# =========================================================
#
# Единый "якорь стиля", который принудительно добавляется к КАЖДОМУ промпту картинки
# (на любой ступени лестницы: оригинал / санитайз / анонимизация / безопасный fallback).
# Плюс из промпта вырезаются слова, заставляющие модель рисовать (illustration/drawing/…).
# Именно из-за них раньше при санитайзе кадры выходили акварельными.
#
# По умолчанию — фотореализм. Можно переопределить через FAST_GEN_STYLE_ANCHOR
# (например, если наоборот нужен рисованный стиль), либо совсем выключить
# принудительный стиль через FAST_GEN_ENFORCE_PHOTOREAL=0.

ENFORCE_PHOTOREAL = _env_bool("FAST_GEN_ENFORCE_PHOTOREAL", True)

STYLE_ANCHOR = os.getenv(
    "FAST_GEN_STYLE_ANCHOR",
    "photorealistic, realistic photography, cinematic film still, shot on camera, "
    "natural lighting, high detail, sharp focus, 8k, "
    "not an illustration, not a drawing, not a painting, not a cartoon",
).strip()

# Слова, которые превращают генерацию в рисунок — вырезаем/заменяем на фото-эквивалент.
ILLUSTRATION_WORDS = re.compile(
    r"\b("
    r"illustrations?|illustrated|drawings?|drawn|sketch(?:es|ed|y)?|"
    r"paintings?|painted|painterly|watercolou?rs?|gouache|"
    r"cartoons?|cartoonish|anime|manga|comics?|"
    r"digital\s+art|concept\s+art|artwork|hand[-\s]?drawn|storybook"
    r")\b",
    re.I,
)
...
# Промпты картинок больше НЕ содержат слова "illustration"; фотореализм добавит enforce_photoreal().
DEFAULT_IMAGE_PROMPT = (
    "A calm, respectful documentary photograph. Neutral peaceful scene, soft natural lighting, "
...
# Слово "illustration" убрано; стиль (фотореализм) добавит enforce_photoreal().
SAFE_IMAGE_FALLBACK = (
    "A calm, respectful, fully clothed historical documentary photograph. "
...
SAFE_SUFFIX = (
    "fully clothed, modest, non-sexualized, calm respectful historical reconstruction, "
    "no violence, no gore, no weapons in use"
)
...
def enforce_photoreal(prompt: str) -> str:
    """ЕДИНАЯ точка нормализации СТИЛЯ для КАЖДОГО промпта картинки.

    1) Вырезает слова-рисунки (illustration/drawing/painting/watercolor/cartoon/…),
       заменяя их на нейтральное "photograph" — иначе модель рисует акварель.
    2) Всегда дописывает STYLE_ANCHOR (по умолчанию — фотореализм).

    Управление через окружение:
      FAST_GEN_ENFORCE_PHOTOREAL=0     -> вообще не трогать стиль (вернуть как есть).
      FAST_GEN_STYLE_ANCHOR="..."      -> свой якорь стиля (например, рисованный).
    """
    text = clean_text(prompt or "")
    if not ENFORCE_PHOTOREAL:
        return text or DEFAULT_IMAGE_PROMPT
    # Убираем слова, которые заставляют модель рисовать.
    text = ILLUSTRATION_WORDS.sub("photograph", text)
    text = clean_text(text)
    if not STYLE_ANCHOR:
        return text or DEFAULT_IMAGE_PROMPT
    anchor_key = STYLE_ANCHOR.split(",", 1)[0].strip().lower()
    if text and anchor_key and anchor_key in text.lower():
        return text
    return f"{text}. {STYLE_ANCHOR}." if text else STYLE_ANCHOR
...
def heuristic_sanitize(text: str, level: int) -> str:
    ...
    out = clean_text(out)
    if level >= 2:
        # ВАЖНО: "reconstruction", а не "illustration" — и без слова, зовущего к рисунку.
        out = f"Calm, respectful, non-violent historical reconstruction. {out}. {SAFE_SUFFIX}."
...
# Единое требование к стилю для LLM — чтобы модель не превращала фото в рисунок.
_LLM_STYLE_RULE = (
    "IMPORTANT: keep the exact same visual medium and style as the original. "
    "The scene must stay a PHOTOREALISTIC, cinematic, real-camera photograph. "
    "Never turn it into a drawing, painting, illustration, watercolor, sketch, cartoon or digital art. "
)
...
def rewrite_keep_character(body: str) -> str:
    ...
    out = _llm([
        {"role": "system", "content": "You edit visual generation prompts to pass strict content filters "
                                       "while staying visual and photorealistic-documentary. " + _LLM_STYLE_RULE},
        {"role": "user", "content": "Rewrite this prompt so it safely passes strict content moderation. "
                                     ...
                                     "People must be fully clothed and non-sexualized. " + _LLM_STYLE_RULE +
...
def rewrite_anonymize(body: str, aliases: List[str], known_names: set) -> str:
    ...
    out = _llm([
        {"role": "system", "content": "You edit prompts so they contain NO named real person and pass strict "
                                       "content filters, while staying a calm documentary scene. " + _LLM_STYLE_RULE},
...
def build_video_payload(prompt: str, image_data_uri: str) -> Dict[str, Any]:
    ...
    # flow-video: keyframes -> inputs[0] трактуется как стартовый кадр (image->video);
    # ultra -> ultra-тариф (1080p/ultra-лимиты) для *_ultra_* операций.
    if VIDEO_KEYFRAMES:
...
def generate_image_once(prompt: str, out_path: Path) -> str:
    ...
    # ЕДИНАЯ точка: любой промпт картинки нормализуем по стилю (фотореализм, без "illustration").
    prompt = enforce_photoreal(prompt)
    url = normalize_base_url(BASE_URL) + V6_GENERATIONS_ENDPOINT
...
def generate_scene_image_with_ladder(out_path: Path, scene: Optional[Scene],
    ...
    ПРИМЕЧАНИЕ: стиль (фотореализм) навешивается внутри generate_image_once -> enforce_photoreal,
    поэтому здесь достаточно управлять только СОДЕРЖАНИЕМ промпта.
    """
    ...
    if aliases:
        ...
        detached = True
        log(f"    {CHARACTER_ATTEMPTS} неудач -> ОТВЯЗЫВАЮ картинку от персонажа ({', '.join(aliases)})")
...
def repair_one(image_path: Path, scene: Optional[Scene], known_names: set,
    ...
    if aliases:
        ...
        detached = True
        log(f"    {CHARACTER_ATTEMPTS} неудач подряд -> ОТВЯЗЫВАЮ от персонажа ({', '.join(aliases)})")
...
def run_video_phase(scenes: Dict[int, Scene], known_names: set,
    ...
    log("\n===== ФАЗА 2: ДОБИВКА ВИДЕО =====")
    log(f"Video operation: {VIDEO_OPERATION} (flow / flow-video, keyframes={VIDEO_KEYFRAMES}, ultra={VIDEO_ULTRA})")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Добивка КАРТИНОК и ВИДЕО до 100% (flow / flow-video) с логикой персонажей "
    ...
    log("ДОБИВКА картинок и видео (flow / flow-video) с логикой персонажей")
    log(f"BASE_URL: {BASE_URL}")
    log(f"Картинки: {IMAGES_DIR}")
    log(f"Видео:    {VIDEOS_DIR}")
    log(f"Image operation: {IMAGE_OPERATION} (flow / nano-banana)")
    log(f"Video operation: {VIDEO_OPERATION} (flow / flow-video, keyframes={VIDEO_KEYFRAMES}, ultra={VIDEO_ULTRA})")
    log(f"Стиль картинок: {'ПРИНУДИТЕЛЬНО -> ' + STYLE_ANCHOR if ENFORCE_PHOTOREAL else 'как в промпте (не навязываю)'}")
