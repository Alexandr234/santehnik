...
# Зум отключён: картинки и видео показываются статично, без приближения.
ZOOM_START = 1.00
ZOOM_END = 1.00
...
################################################
# 3. РЕНДЕР КАДРОВ: ВИДЕО/КАРТИНКИ БЕЗ ЗУМА + БЕЗ ПЕРЕХОДОВ
################################################
...
def zoom_for_frame(frame_index: int, total_frames: int) -> float:
    """Зум отключён: всегда возвращаем 1.0, картинка показывается статично."""
    return 1.0
...
def iter_image_frames(segment: Segment, total_frames: int) -> Iterator[np.ndarray]:
    ...
    # Подгоняем картинку под 1920x1080 и показываем её статично, без зума.
...
def build_video_for_language(lang: str, audio_path: Path, visual_dir: Path) -> bool:
    ...
    try:
        ...
        log("Переходы: отключены. Зум: отключён, картинки статичны. Субтитры: отключены.")
