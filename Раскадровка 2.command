#!/bin/bash
# ============================================================================
#  Раскадровка 2.command — двойной клик, ничего ставить не нужно.
#
#  Скринит каждое видео из папки каждые 0.5 секунды (120 кадров/минуту),
...
INTERVAL=0.5        # секунд между кадрами
PER_MIN=120         # кадров в минуту (для справки в выводе)
WIDTH=720           # ширина кадра в пикселях (для компактности)
COLS=6              # колонок в PDF-сетке
ROWS=10             # строк в PDF-сетке (6x10 = 60 кадров = 30 секунд на страницу)
...
echo "============================================================"
echo "  Раскадровка видео (каждые 0.5 секунды)"
echo "============================================================"
...
OUT_ROOT="$INPUT/_РАСКАДРОВКА_0.5с"
mkdir -p "$OUT_ROOT"

echo "📂 Видео из:   $INPUT"
echo "🖼  Интервал:  каждые ${INTERVAL} c  ->  ${PER_MIN} кадров/минуту"
...
def human_time(seconds):
    # таймкоды с точностью до десятых (для интервалов меньше секунды)
    total = round(seconds * 10) / 10
    whole = int(total)
    tenths = int(round((total - whole) * 10))
    m, s = divmod(whole, 60)
    h, m = divmod(m, 60)
