# -*- coding: utf-8 -*-
"""
Оживление локальных картинок в видео через media_gen_api V6 без storage upload.
...
Что изменилось относительно V4-версии (новая API-документация media_gen_api V6):
   - старый endpoint /api/v4/flow/video/from-ingredients и /api/v4/operations/...
     больше не используются;
   - генерация идёт через единый endpoint POST /api/v6/generations
     с canonical operation id flow_video_from_ingredients;
   - картинка-ингредиент передаётся в inputs[] (data URI прямо в запросе);
   - aspect_ratio теперь в форме n:n (16:9), а не старый enum VIDEO_ASPECT_RATIO_*;
   - статус запрашивается через GET /api/v6/generations/{generation_id},
     статусы queued/running/succeeded/failed;
   - результат приходит в results[] как download_url (файл) либо inline data.
"""
...
# Canonical V6 operation id для image->video из ингредиентов (см. GET /api/v6/capabilities).
...
# По схеме API для inline data URI максимум 5 MB на одну картинку.
...
def extract_generation_id(data: Dict[str, Any]) -> Optional[str]:
    candidates = [
        ...
        data.get("data", {}).get("operation_id") if isinstance(data.get("data"), dict) else None,
...
def extract_video_source(data: Dict[str, Any]) -> str:
    # V6: результат приходит в results[] как inline data (data:video/...) или download_url.
    ...
    # Фолбэк: рекурсивно ищем data:video/... или http(s)-ссылку в любом месте ответа.
    all_strings = recursive_find_strings(data)
    ...
    raise RuntimeError(
        f"Не удалось найти video source в ответе: {json.dumps(data, ensure_ascii=False)[:2000]}"
    )
...
def poll_operation(generation_id: str, scene_index: int) -> Dict[str, Any]:
    ...
    while True:
        ...
        if status in {"failed", "error", "cancelled", "canceled"}:
            raise RuntimeError(
                f"[ERROR] {scene_index:04d}: генерация {generation_id} завершилась со статусом {status}. Ответ: {data}"
            )
