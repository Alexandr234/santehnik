...
"""
Оркестратор для последовательного запуска пайплайна из ШЕСТИ скриптов:

  1) master_prompt_pipeline_children_enabled_no_famous_APIKEY_FIXED_v4ДЕТИ.py
  2) СКРИПТ ГЕНЕРАЦИЯ КАРТИНОК/media_gen_character_image_pipeline_flower.py
  3) ВИДЕО/video_from_local_images_base64.py
  4) video_repair_characters_100percent.py
  5) ПЕРЕВОД+ПЕРЕОЗВУЧКА/translate_and_voice.py
  6) audio_video_sync_montage.py

Что делает:
- запускает скрипты строго по очереди в указанном выше порядке;
...
Запуск:
    python3 orchestrator_pipeline_v4.py
...
PIPELINE = [
    {
        # 1) Генерация текстовых промптов/сценариев (в т.ч. «дети»).
        "name": "MASTER_PROMPTS",
    ...
    {
        # 2) Генерация картинок персонажей (media_gen ... flower).
        "name": "CHARACTER_IMAGES",
        "path": f"{BASE}/СКРИПТ ГЕНЕРАЦИЯ КАРТИНОК/media_gen_character_image_pipeline_flower.py",
        "output_globs": [
            f"{BASE}/СКРИПТ ГЕНЕРАЦИЯ КАРТИНОК/**/*.png",
            f"{BASE}/СКРИПТ ГЕНЕРАЦИЯ КАРТИНОК/**/*.jpg",
            f"{BASE}/СКРИПТ ГЕНЕРАЦИЯ КАРТИНОК/**/*.jpeg",
            f"{BASE}/СКРИПТ ГЕНЕРАЦИЯ КАРТИНОК/output/**/*.png",
            f"{BASE}/СКРИПТ ГЕНЕРАЦИЯ КАРТИНОК/images/**/*.png",
        ],
    },
    {
        # 3) Генерация видео из локальных картинок (base64).
        "name": "VIDEO_GENERATION",
        "path": f"{BASE}/ВИДЕО/video_from_local_images_base64.py",
        "output_globs": [
            f"{BASE}/ВИДЕО/**/*.mp4",
            f"{BASE}/ВИДЕО/output/**/*.mp4",
            f"{BASE}/ВИДЕО/videos/**/*.mp4",
        ],
    },
    {
        # 4) «Починка» персонажей в видео до 100%.
        #    По умолчанию НЕ пропускаем (пусть всегда доводит предыдущие видео),
        #    т.к. надёжно отличить «починенное» видео от «сырого» по имени файла
        #    нельзя. При необходимости задайте свой output_globs.
        "name": "VIDEO_REPAIR",
        "path": f"{BASE}/video_repair_characters_100percent.py",
        "output_globs": [],
    },
    {
        # 5) Перевод + переозвучка.
        "name": "TRANSLATE_VOICE",
        "path": f"{BASE}/ПЕРЕВОД+ПЕРЕОЗВУЧКА/translate_and_voice.py",
        "output_globs": [
            f"{BASE}/ПЕРЕВОД+ПЕРЕОЗВУЧКА/**/*.mp3",
            f"{BASE}/ПЕРЕВОД+ПЕРЕОЗВУЧКА/**/*.wav",
            f"{BASE}/ПЕРЕВОД+ПЕРЕОЗВУЧКА/**/*.m4a",
            f"{BASE}/ПЕРЕВОД+ПЕРЕОЗВУЧКА/output/**/*.mp4",
        ],
    },
    {
        # 6) Финальный монтаж: синхронизация аудио и видео.
        #    По умолчанию НЕ пропускаем — это финальная сборка, обычно нужна всегда.
        #    При необходимости задайте output_globs с путём к итоговому ролику.
        "name": "FINAL_MONTAGE",
        "path": f"{BASE}/audio_video_sync_montage.py",
        "output_globs": [],
    },
]
